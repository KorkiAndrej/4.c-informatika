...
def spremi_proizvode_csv(lista_ucenika, Spremljeni_ucenici):
    """Sprema listu proizvoda u CSV datoteku."""
    with open(Spremljeni_ucenici, mode='w', newline='', encoding='utf-8') as datoteka:
    ...
    print(f"Spremljeno u {Spremljeni_ucenici}")

def ucitaj_proizvode_csv(Spremljeni_ucenici):
    """Učitava proizvode iz CSV datoteke i vraća listu objekata."""
    ucitani_ucenici = []
    with open(Spremljeni_ucenici, mode='r', encoding='utf-8') as datoteka:
        reader = csv.DictReader(datoteka)
        for red in reader:
            p = Ucenik(red['ime'], (red['prezime']), (red['razred']))
            ucitani_ucenici.append(p)
    print(f"Učitano iz {Spremljeni_ucenici}")
